...
##############################################################################
# Fully-adapted particle filter for the linear Gaussian SSM_Xiongming Dai with D
##############################################################################
particleFilterD <- function(y, theta, noParticles, initialState,D) {
  ...
  for (t in 2:T) {
    ...
    #introduce D Xiongming dai
    for (tt in 1:noParticles) {
      if(as.integer(length(D))==0)
       as.integer(length(D))=10
      r=tt%%(as.integer(length(D)))
      #thetaProposed[k, ]<-D[r+1,]
      EnergyParticles[tt]<-D[r+1]
    }
    ...
    particles[, t] <- part1 * part2 +  sqrt(part1)*EnergyParticles   
    ...
    if((t==T)){
      if(as.integer(T/2)==0)
        T=T+1
      pdf(file="QQnorm_example1_XMD.pdf")
      plot(qnorm(((1:noParticles)-.5)/noParticles),sort(particles[, T/2]),xlab="Theoretical Quantiles", ylab="Sample Quantiles", main="Normal(0,1)",col="#9933FF")
      dev.off()
    }#particles[, t] <- part1 * part2 +  sqrt(part1)*EnergyParticles   
  ...
  list(xHatFiltered = xHatFiltered,
       ...
       particles = particles,
       weights = normalisedWeights,EnergyParticles=EnergyParticles)
