...
particleMetropolisHastingsD <- function(y, initialPhi, sigmav, sigmae,
  noParticles, initialState, noIterations, stepSize,D) {
    ...
    for (k in 2:noIterations) {
      ...
      if(as.integer(length(D))==0)
       as.integer(length(D))=10
      r=k%%(as.integer(length(D)))
      #thetaProposed[k]<-D[r+1]

      
      phiProposed[k] <- phi[k - 1] + stepSize * D[r+1]
...
particleMetropolisHastingsSVmodel <- function(y, initialTheta, noParticles,
  ...
  for (k in 2:noIterations) {
    ...
    if(as.integer(length(D)/3)==0)
      as.integer(length(D)/3)=10
    r=k%%(as.integer(length(D)/3))
    thetaProposed[k, ]<-D[r+1,]
    # Estimate the log-likelihood (don't run if unstable system)
...
###################################################################################################################
################################  The original without D  ######################################################
################################################################################################################

particleMetropolisHastingsSVmodel_withoutD<- function(y, initialTheta, noParticles,noIterations, stepSize,theta_SAVER) {
#particleMetropolisHastingsSVmodel <- function(y, initialTheta, noParticles,
#  noIterations, stepSize) {
  ...
  cat(sprintf("The theta_SAVER:                 %.4f \n", theta_SAVER))
  cat(sprintf(" The theta_SAVER_[100,1]:                 %.4f \n", theta_SAVER[100,1]))
  
  write.table( theta_SAVER, file="thata_SAVER1_2_3.txt", quote=F)
  
  
  write.table( theta_SAVER[,1], file="thata_SAVER1.txt", quote=F)
  hist(theta_SAVER[,1]);
  
  write.table( theta_SAVER[,2], file="thata_SAVER2.txt", quote=F)
  hist(theta_SAVER[,2]);
  
  write.table( theta_SAVER[,3], file="thata_SAVER3.txt", quote=F)
  hist(theta_SAVER[,3]);
  
  list(theta = theta, xHatFiltered = xHatFiltered, proposedThetaAccepted = proposedThetaAccepted,theta_SAVER=theta_SAVER)
}
